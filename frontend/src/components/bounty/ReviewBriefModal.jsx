...
export default function ReviewBriefModal({ record, onClose }) {
  ...
  // Compute factual metadata flags dynamically
  const flags = [];
  ...
  if (notes.trim() !== '') {
    flags.push({ label: 'REVIEWER NOTES PRESENT', desc: 'Custom reviewer annotations attached' });
  }
  if (clauses.length > 1) {
  ...
  const checklist = [
    ...
    { label: 'Jurisdiction Reviewed', done: Boolean(record.jurisdiction) },
    ...
    { label: 'Clauses Reviewed', done: clauses.length > 0 },
    { label: 'Verification Status Assigned', done: status !== 'DRAFT' },
    { label: 'Reviewer Notes Reviewed', done: notes.trim() !== '' }
  ];
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-slate-950/80 backdrop-blur-sm animate-fade-in overflow-y-auto print:p-0 print:bg-white print:static print:block">
      ...
      {/* Modal Shell */}
      <div className="print-container bg-[#0B0F19] border border-slate-800 rounded-lg max-w-4xl w-full max-h-[90vh] flex flex-col shadow-2xl overflow-hidden my-8 print:my-0 print:border-none print:shadow-none">
        
        {/* Modal Top Bar */}
        <div className="p-4 sm:px-6 border-b border-slate-800 bg-[#0E1524] flex items-center justify-between gap-4 print:hidden">
        ...
        <div className="flex-1 overflow-y-auto p-6 sm:p-8 space-y-8 text-slate-200 font-sans print:overflow-visible print:p-4 print:text-black">
          
          {/* Document Header & Title Block */}
          <div className="border-b border-slate-800 print-border-dark pb-6 space-y-4">
            
            {/* Synthetic Disclaimer Banner */}
            <div className="flex items-center gap-2 text-xs font-mono text-amber-400 print:text-amber-700 bg-amber-950/40 print:bg-amber-50 border border-amber-800/60 print:border-amber-300 px-3.5 py-2 rounded">
            ...
            <div className="flex flex-wrap items-center justify-between gap-3 pt-2">
              <div>
                <span className="text-xs font-mono font-bold text-amber-400 print:text-slate-800 tracking-wider">
                  LEGAL REVIEW DOSSIER ID: {record.id}
                </span>
              ...
              <div className="flex items-center gap-2">
                ...
                <span className="text-xs font-mono font-bold px-2.5 py-1 rounded bg-slate-900 print:bg-slate-100 border border-slate-800 print:border-slate-300 text-slate-300 print:text-slate-800">
                  RISK: {risk}
                </span>
              </div>
            </div>
          ...
          <section className="space-y-3">
            ...
            <div className="grid grid-cols-2 sm:grid-cols-3 gap-4 p-4 rounded-lg bg-slate-900/50 print-bg-light border border-slate-800 print-border-dark text-xs font-mono">
              ...
              <div>
                ...
                <span className="text-amber-400 print:text-slate-900 font-bold">{risk} RISK</span>
              </div>
            </div>
          </section>

          {/* SECTION 02: DOCUMENT CLAUSES */}
          <section className="space-y-3">
            <div className="flex items-center gap-2 text-xs font-mono font-bold text-amber-400 print:text-slate-900 uppercase tracking-widest border-b border-slate-800/80 print-border-dark pb-2">
          ...
          <section className="space-y-3">
            ...
            {flags.length > 0 ? (
              ...
              <p className="text-xs font-mono text-slate-400 italic p-3 bg-slate-900/30 rounded border border-slate-800">
                NO ACTIVE REVIEW FLAGS IDENTIFIED
              </p>
            )}
          </section>

          {/* SECTION 05: REVIEW CHECKLIST */}
          <section className="space-y-3">
            <div className="flex items-center gap-2 text-xs font-mono font-bold text-amber-400 print:text-slate-900 uppercase tracking-widest border-b border-slate-800/80 print-border-dark pb-2">
          ...
          {/* SECTION 06: REVIEWER OBSERVATIONS */}
          <section className="space-y-3">
            <div className="flex items-center gap-2 text-xs font-mono font-bold text-amber-400 print:text-slate-900 uppercase tracking-widest border-b border-slate-800/80 print-border-dark pb-2">
              <span className="text-slate-500">06</span>
              <span>REVIEWER OBSERVATIONS</span>
            </div>
        ...
        <div className="p-4 sm:px-6 border-t border-slate-800 bg-[#0E1524] flex items-center justify-between text-xs font-mono text-slate-400 print:hidden">
          <div className="flex items-center gap-2">
            ...
            <span>Legal Review Brief & Dossier Ready for Export</span>
          </div>
